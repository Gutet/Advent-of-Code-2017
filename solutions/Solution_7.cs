...
namespace AdventOfCode.solutions
{
    public static class Solution_7
    {
        ...
        {
            #region input
            var input = @"fjkfpm (69) -> kohxzh, liwvq, eqkio, xvoyybs
dsiixv (52)
fhimhm (66)
mdlubuq (73)
ulobwyb (41)
        ...
        private static string getWeightAbnormality(List<programNode> programNodes, string startProgram)
        {
            ...
            foreach (var item in weightLinq)
            {
                ...
                for (int i = 1; i < item.weights.Count; i++)
                {
                    ...
                    {
                        var mainNodeToChange = programNodes.Where(x => x.programName == item.weights[i].programName).First();
                        var originalProgramWeight = mainNodeToChange.programWeight - mainNodeToChange.legWeight;
